...
`aspects/devotion/schedule.json` にルーティンプールを定義。`notion-daily-plan.ts` はこのファイルを読み、確定予定の空き時間にルーティンを自動配置する。
...
1. 月曜〜昨日の devotion DB 実績を取得（完了エントリのみ）
...
devotion DB のエントリタイトルと schedule.json の label を前方一致で照合する。例: 「開発 @ 図書館」→「開発」にマッチ。
