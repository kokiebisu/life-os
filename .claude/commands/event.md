...
3. **日時を決定する** — 以下を考慮:
   ...
   - **理想のルーティン** — `aspects/devotion/CLAUDE.md` を参照して空き時間を判断する
...
**schedule.json に定義されたルーティンは必ず devotion DB に入れる（例外あり）:**
- `schedule.json` の `routines` に定義されている活動（開発・ジム等）は devotion DB
